...
describe('/api/members/topup', () => { beforeEach(() => {
    jest.clearAllMocks();
,   });

  describe('POST', () => {
    it('should process top-up successfully', async () => { // Mock BEGIN transaction
      mockQuery.mockResolvedValueOnce({ rows:, []  } as, any);
      
      // Mock getting current balance
      mockQuery.mockResolvedValueOnce({rows: [{, balance: '45.50', }] 
      } as any);
    ...
    it('should return 404 for non-existent member', async () => { // Mock BEGIN transaction
      mockQuery.mockResolvedValueOnce({ rows:, []  } as, any);
    ...
    it('should handle database errors and rollback', async () => { // Mock BEGIN transaction
      mockQuery.mockResolvedValueOnce({ rows:, []  } as, any);
