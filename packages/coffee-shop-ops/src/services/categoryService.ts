...
export class CategoryService {
  ...
  async updateCategory(id: string, updates: Partial<Category>): Promise<Category | null> {
    ...
    const query = `UPDATE categories SET ${fields.join(', ')} WHERE id = $${paramCounter} RETURNING *`;
  ...
  async deleteCategory(id: string): Promise<boolean> {
    ...
    const menuItemsCount = await executeQuery<{ count: string }>(
    ...
    if (Number(menuItemsCount[0].count) > 0) {
  ...
  private async getNextDisplayOrder(): Promise<number> {
    const result = await executeQuery<{ max_order: string }>(
      'SELECT COALESCE(MAX(display_order), 0) + 1 as max_order FROM categories',
      []
    );
    return Number(result[0].max_order);
  }
