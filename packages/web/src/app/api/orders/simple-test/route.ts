...
export async function, POST(request:, NextRequest) {
  try {
    ...
    const { customer_name, customer_email,, member_id, items,, order_type, =, 'takeout',, notes, payment_method, =, 'wechat_pay' }
    ...
    // Calculate, total amount, const total_amount = items.reduce((sum: number, item: any) => 
    ...
    // Create, order items, for (const item of, items) {
      await query(
